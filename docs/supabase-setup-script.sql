-- Konectr Supabase Database Setup Script
...
-- RLS Policy: Only authenticated admins can view all data
...
-- Create security logs table (admin only)
CREATE TABLE security_logs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  
  -- Event details
  event_type VARCHAR(50) NOT NULL, -- 'login', 'data_access', 'suspicious_activity', 'data_export'
  ip_address INET,
  user_agent TEXT,
  details JSONB,
...
-- Enable RLS for security logs (admin only)
ALTER TABLE security_logs ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Only admins can view security logs
CREATE POLICY "Admin only security logs" ON security_logs
